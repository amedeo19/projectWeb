<?php
  ...
  include './../html/utilities/functions.php';
    ...
    if (isset($_POST['nome']) && isset($_POST['cognome']) && isset($_POST['email'])
      && isset($_POST['password']) && isset($_POST['ripetiPassword']) && isset($_POST['città']) && isset($_POST['telefono'])) {
      $email = $_POST['email'];
      $pwd = $_POST['password'];
      $mysqli = connectToDatabase();

      if ($mysqli->connect_error) {
        header('Location:Nuovo_cliente.php?error=-3');
        exit;
      }
      ...
      if ($res) {
        header('Location:Nuovo_cliente.php?error=-1');
        exit;
      }
      ...
      if (!isset($_FILES['fotoProfilo']) || !is_uploaded_file($_FILES['fotoProfilo']['tmp_name'])) {
        ...
        if (!$is_img) {
            header('Location:Nuovo_cliente.php?error=-4');
            exit;
        } else {
      ...
      if (!$res) {
        ...
        header('Location:Admin_lista_clienti.php?error=1');
      }
      ...
      */?>
      ...
      header('Location:Nuovo_cliente.php?error=-2');
    }
?>
