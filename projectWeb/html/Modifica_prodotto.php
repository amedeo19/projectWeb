...
$_SESSION["lastPage"] = 'Modifica_prodotto.php';
$fornitore = 2;
...
$query_sql="SELECT * FROM prodotto WHERE prodotto.e_mail_fornitore  = '" . $e_mail_f . "' AND prodotto.nomeProdotto  = '" . $nomeP . "'";
...
$query_sql3="SELECT * FROM prodotto WHERE e_mail_fornitore = '" . $e_mail_f . "'";
...
function EditProdotto($data, $img){
      $conn = connectToDatabase();
      $e_mail_fornitore = $data["e_mail"];
      $nomeP = $data["nomeP"];
      $prezzo = $data["prezzo"];
      $desc = $data["desc"];
      $image = $img;

      $query_sql="UPDATE prodotto SET prodotto.prezzo='" . $prezzo . "', prodotto.descrizione='" . $desc . "', prodotto.img='" . $image . "'
                  WHERE prodotto.e_mail_fornitore='" . $e_mail_fornitore . "' AND prodotto.nomeProdotto='" . $nomeP . "'";
      $result = $conn->query($query_sql);
  ...
  if(!isset($_FILES['immagine']) || !is_uploaded_file($_FILES['immagine']['tmp_name'])){
    ...
    if (!$is_img) {
        ...
        if (!file_exists($target_file)) {
            ...
            if (move_uploaded_file($userfile_tmp, "$imgDir/$e_mail_f.$num.$defaultFormat")) { // criptare anche le immagini delle mail
                //Se l'operazione è andata a buon fine...

                $img="$e_mail_f.$num.$defaultFormat";
     ...
     <div class="container-fluid py-3 text-white bg-secondary">
           ...
           <main>
             <section id="schermata_di_registrazione" style="margin-bottom:25px;">
       				<form method="post" action="Modifica_prodotto.php" id="inputform" onsubmit="return validateForm()"  enctype="multipart/form-data">
                 <fieldset class= "border border-light mt-2" id="registrazione">
                   <legend class="w-50 text-center">Modifica prodotto</legend>
                   <div class="card card-body bg-secondary">
           				  <div class="form-group">
             					<label for="inputNome">NOME: <?php echo $myProdotto->nomeProdotto ?> </label>
           				  </div>
           				  <div class="form-group">
             					<label for="inputPrezzo">PREZZO</label>
             					  <input type="text" name="prezzo" class="form-control"  placeholder="<?php echo $myProdotto->prezzo ?> €" required></input>
           				  </div>
                     <div class="form-group">
                       <label for="inputEmail">DESCRIZIONE</label>
                         <input type="text" name="desc" class="form-control" placeholder="<?php echo $myProdotto->descrizione ?>" required></input>
                     </div>
                     <div class="form-group">
                 ...
                 <div class="registrazione col-md-6 offset-md-3">
                   ...
                   <input type="hidden" name="nomeP" value="<?php echo $myProdotto->nomeProdotto ?>" />
                   <input type="hidden" name="e_mail" value="<?php echo $myProdotto->e_mail_fornitore ?>" />
